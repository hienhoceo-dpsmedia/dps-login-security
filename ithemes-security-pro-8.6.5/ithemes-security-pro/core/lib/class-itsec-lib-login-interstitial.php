...
class ITSEC_Lib_Login_Interstitial {
	...
	public function handle_token_expired( $errors ) {
		...
		if ( isset( $_GET[ self::R_EXPIRED ] ) ) {
			$errors->add(
				...
				esc_html__( 'Sorry, this request has expired. Please log in again.', 'it-l10n-ithemes-security-pro' )
			);
		}
	...
	public function async_action() {
		...
		$result = wp_parse_args( $result, array(
			'message' => esc_html__( 'Action processed. Please continue in your original browser.', 'it-l10n-ithemes-security-pro' ),
			'title'   => esc_html__( 'Action Processed', 'it-l10n-ithemes-security-pro' ),
		) );
	...
	public function ajax_handler() {
		...
		if ( empty( $this->registered[ $slug ] ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Invalid Interstitial Action', 'it-l10n-ithemes-security-pro' ) ) );
		}
	...
	private function interim_login() {
		...
		login_header( '', '<p class="message">' . __( 'You have logged in successfully.', 'it-l10n-ithemes-security-pro' ) . '</p>' );
		?>
		</div>
		<?php
