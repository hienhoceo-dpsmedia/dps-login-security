...
class ITSEC_Lib_Config_File {
	...
	public static function get_minimal_server_config() {
		...
		$comment_delimiter = self::get_comment_delimiter( $server );
		$modification = "$comment_delimiter " . __( 'Solid Security preserved the following settings as removing them could prevent the site from functioning correctly.', 'it-l10n-ithemes-security-pro' ) . "\n$modification";
...
//			return new WP_Error( 'itsec-lib-config-file-server-config-file-updates-disabled', __( 'Updates to the server config file are disabled via a filter.', 'it-l10n-ithemes-security-pro' ) );
	...
	protected static function update( $file, $type, $modification, $clear_existing_modifications = true ) {
		...
		if ( ! $contents && in_array( $type, array( 'apache', 'wp-config' ), true ) ) {
			...
			$error = new WP_Error( "itsec-config-file-update-empty::{$type}", sprintf(
				__( 'Empty file encountered when attempting to update <code>%1$s</code>. Manual configuration for the <code>%1$s</code> file can be found on the Security > Settings page in the Advanced section.', 'it-l10n-ithemes-security-pro' ),
				$display_file
			) );
	...
	protected static function get_prepared_modification( $modification, $comment_delimiter ) {
		...
		$modification  = "$comment_delimiter BEGIN Solid Security - " . __( 'Do not modify or remove this line', 'it-l10n-ithemes-security-pro' ) . "\n";
		$modification .= "$comment_delimiter Solid Security Config Details: " . self::FORMAT_VERSION . "\n";
		$modification .= "$supplied_modification\n";
		$modification .= "$comment_delimiter END Solid Security - " . __( 'Do not modify or remove this line', 'it-l10n-ithemes-security-pro' );
