...
export function* activateModule( module ) {
	try {
		...
		yield createNotice(
			'success',
			__( 'Activated feature', 'it-l10n-ithemes-security-pro' ),
			{
				type: 'snackbar',
			}
		);
...
export function* deactivateModule( module ) {
	try {
		...
		yield createNotice(
			'success',
			__( 'Deactivated feature', 'it-l10n-ithemes-security-pro' ),
			{
				type: 'snackbar',
			}
		);
...
export function* saveSettings( modules = true, validate = false ) {
	...
	if ( success.length ) {
		yield createNotice( 'success', __( 'Saved settings.', 'it-l10n-ithemes-security-pro' ), {
			type: 'snackbar',
		} );
