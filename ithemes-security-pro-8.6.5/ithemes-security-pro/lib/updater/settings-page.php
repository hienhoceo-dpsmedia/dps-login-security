...
class Ithemes_Updater_Settings_Page {
	...
	private function license_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to license products.', 'it-l10n-ithemes-security-pro' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to license products.', 'it-l10n-ithemes-security-pro' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to license products.', 'it-l10n-ithemes-security-pro' );
	...
	private function license_patchstack_site( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to remove patchstack licenses.', 'it-l10n-ithemes-security-pro' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to remove patchstack licenses.', 'it-l10n-ithemes-security-pro' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to remove patchstack licenses.', 'it-l10n-ithemes-security-pro' );
		}

		$keys = Ithemes_Updater_Keys::get();
		if ( empty( $keys['ithemes-security-pro'] ) ) {
			$this->errors[] = __( 'Unable to locate API key for Solid Security Pro.', 'it-l10n-ithemes-security-pro' );
		}
		...
		if ( ! empty( $success ) ) {
			$this->messages[] = wp_sprintf( _n( 'Successfully added Patchstack license to %l.', 'Successfully removed Patchstack licenses from %l sites.', count( $success ), 'it-l10n-ithemes-security-pro' ), $success );
		}

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason ) {
				$this->errors[] = sprintf( __( 'Unable to add patchstack license to %1$s. Reason: %2$s', 'it-l10n-ithemes-security-pro' ), $name, $reason );
			}
		}
	}
	...
	private function unlicense_patchstack_sites( $data ) {
		...
		if ( ! empty( $success ) ) {
			$this->messages[] = wp_sprintf( _n( 'Successfully removed Patchstack license from %l site.', 'Successfully removed Patchstack licenses from %l sites.', count( $success ), 'it-l10n-ithemes-security-pro' ), $success );
		}

		if ( ! empty( $fail ) ) {
			foreach ( $fail as $name => $reason ) {
				$this->errors[] = sprintf( __( 'Unable to remove patchstack license from %1$s. Reason: %2$s', 'it-l10n-ithemes-security-pro' ), $name, $reason );
			}
		}
	...
	private function unlicense_packages( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to remove licenses.', 'it-l10n-ithemes-security-pro' );
		} else if ( empty( $data['username'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username in order to remove licenses.', 'it-l10n-ithemes-security-pro' );
		} else if ( empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership password in order to remove licenses.', 'it-l10n-ithemes-security-pro' );
		} else if ( empty( $data['packages'] ) ) {
			$this->errors[] = __( 'You must select at least one license to remove. Ensure that you select the licenses that you wish to remove in the listing below.', 'it-l10n-ithemes-security-pro' );
		}
	...
	<div class="solidwp-licensing">
		<div class="solidwp-licensing-wrap">
			<div class="solidwp-licensing-wrap-header">
				<h2><?php _e( 'SolidWP Licensing', 'it-l10n-ithemes-security-pro' ); ?></h2>
			...
			<table class="ithemes-updater-listing widefat">
				<thead>
					<tr>
						...
						<th scope="col" class="mobile-hidden"><?php _e( 'Member', 'it-l10n-ithemes-security-pro' ); ?></th>
						<th scope="col"><?php _e( 'Product Status', 'it-l10n-ithemes-security-pro' ); ?></th>
						<th scope="col" class="mobile-hidden"><?php _e( 'Expiration', 'it-l10n-ithemes-security-pro' ); ?></th>
						<th scope="col" class="mobile-hidden"><?php _e( 'Remaining Licenses', 'it-l10n-ithemes-security-pro' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<?php $count = 0; ?>
...
?>
	...
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products border-bottom" id="ithemes-updater-licensed">
			<div class="solidwp-table-header">
				<div>
					<h3 class="subtitle"><?php _e( 'Patchstack Enabled', 'it-l10n-ithemes-security-pro' ); ?></h3>
				</div>
			</div>
			...
			<table class="ithemes-updater-listing widefat">
				<tbody>
					<tr>
						<td colspan="2">
							<p><?php printf( __( 'You have %d Remaining Site Licenses', 'it-l10n-ithemes-security-pro' ), $patchstack_available ); ?></p>
							<p><strong><?php _e( 'This site is licensed with Patchstack.', 'it-l10n-ithemes-security-pro' ); ?></strong></p>
							<p><a href="https://my.solidwp.com/panel/patchstack.php"><?php _e( 'Click here to manage your Patchstack licenses.', 'it-l10n-ithemes-security-pro' ); ?></a></p>
						</td>
					</tr>
					<?php $count = 0; ?>
				...
				<tfoot>
					<tr>
						<td colspan="2">
							<input type="text" name="it-updater-username" placeholder="SolidWP Username" value="<?php echo esc_attr( $post_data['username'] ); ?>" autocomplete="off" />
							<input type="password" name="it-updater-password" placeholder="Password" value="<?php echo esc_attr( $post_data['password'] ); ?>" />
							<input class="button-primary" type="submit" name="submit" value="<?php _e( 'Remove Patchstack License', 'it-l10n-ithemes-security-pro' ); ?>" />
	...
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>" autocomplete="off">
		<div class="ithemes-updater-products" id="ithemes-updater-licensed">
			<div class="solidwp-table-header border-bottom">
				<div>
					<h3 class="subtitle"><?php _e( 'Enable Patchstack', 'it-l10n-ithemes-security-pro' ); ?></h3>
				</div>
			</div>
			<table class="ithemes-updater-listing widefat">
				<tbody>
					<tr><td>
						<p class="unlicensed-patchstack-paragraph"><?php printf( __( 'You have <strong>%d Remaining Site Licenses</strong>', 'it-l10n-ithemes-security-pro' ), $patchstack_available ); ?></p>
						<p><?php _e( '<strong>This site is not licensed with Patchstack.</strong>', 'it-l10n-ithemes-security-pro' ); ?></p>
						<p><?php printf( __( 'License your site below using your SolidWP username and password or <a href="%s">click here to manage your Patchstack licenses.</a>', 'it-l10n-ithemes-security-pro' ), 'https://my.solidwp.com/panel/patchstack.php' ); ?></p>
					</td></tr>
				</tbody>
				<tfoot>
					<tr>
						<td>
							...
							<input type="text" name="it-updater-username" placeholder="SolidWP Username" value="<?php echo esc_attr( $post_data['username'] ); ?>" autocomplete="off" />
							<input type="password" name="it-updater-password" placeholder="Password" value="<?php echo esc_attr( $post_data['password'] ); ?>" />
							<input class="button-secondary" type="submit" name="submit" value="<?php _e( 'License This Site', 'it-l10n-ithemes-security-pro' ); ?>" />
...
?>
	<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>" autocomplete="off">
		...
		<div class="ithemes-updater-products" id="ithemes-updater-unlicensed">
			<div class="solidwp-table-header">
				<div>
					<h3><?php _e( 'Unlicensed Products', 'it-l10n-ithemes-security-pro' ); ?></h3>
					...
					<p><?php _e( 'To license products, select the products you wish to license, enter your SolidWP membership username and password, and press the License Products button.', 'it-l10n-ithemes-security-pro' ); ?></p>
					<p><?php printf( __( 'Need help? <a href="%s">Click here for a quick video tutorial</a>.', 'it-l10n-ithemes-security-pro' ), 'https://go.solidwp.com/solidwp-updater' ); ?></p>
				</div>
			</div>
...
?>
	<div class="ithemes-updater-products" id="ithemes-updater-unrecognized">
		<div class="solidwp-table-header">
			<div>
				<h3><?php _e( 'Unrecognized Products', 'it-l10n-ithemes-security-pro' ); ?></h3>
				...
				<p><?php printf( __( 'Please check this page again at a later time to see if the problem resolves itself. If the product remains, please contact <a href="%s">SolidWP support</a> and provide them with the details given below.', 'it-l10n-ithemes-security-pro' ), 'https://solidwp.com/support/' ); ?></p>
			</div>
		</div>
		<table class="ithemes-updater-listing widefat">
			<thead>
...
?>
	<div class="wrap" id="ithemes-updater-relicense">
		...
		<div class="solidwp-licensing-wrap">
			...
			<p><?php printf( __( 'The licenses on this site are for <code>%s</code>.', 'it-l10n-ithemes-security-pro' ), esc_html( $site_url_from_server ) ); ?></p>
			...
			<form id="posts-filter" enctype="multipart/form-data" method="post" action="<?php echo esc_attr( $this->self_url ); ?>">
				...
				<div id="ithemes-updater-settings">
					<div id="ithemes-updater-table-wrapper">
						<h3><?php _e( 'License Option', 'it-l10n-ithemes-security-pro' ); ?></h3>
						<p>
							<label>
						...
						<p>
							<label>
								...
								<?php printf( __( 'Change the existing licenses to be for <code>%s</code>.', 'it-l10n-ithemes-security-pro' ), esc_html( $data['site_url'] ) ); ?>
							</label>
						</p>
						<p class="description ithemes-updater-description-notice">
							<span class="dashicons dashicons-warning"></span>
							<?php printf( __( 'Note: If the <code>%s</code> site still exists and is different from this site, you will have to create new licenses on that site.', 'it-l10n-ithemes-security-pro' ), esc_html( $data['site_url'] ) ); ?>
						</p>
						<p class="description ithemes-updater-description-warning secondary-description"><?php printf( __( 'Use this option if this site\'s primary URL has changed from <code>%1$s</code> to <code>%2$s</code>.', 'it-l10n-ithemes-security-pro' ), esc_html( $site_url_from_server ), esc_html( $data['site_url'] ) ); ?></p>
						<div class="input-container">
							<label for="it-updater-username"><?php _e( 'SolidWP Username', 'it-l10n-ithemes-security-pro' ); ?></label>
							<input id="it-updater-username" type="text" name="it-updater-username" value="<?php echo esc_attr( $data['username'] ); ?>" />
							<label for="it-updater-password"><?php _e( 'SolidWP Password', 'it-l10n-ithemes-security-pro' ); ?></label>
...
<?php
	...
	private function relicense( $data ) {
		...
		if ( empty( $data['username'] ) && empty( $data['password'] ) ) {
			$this->errors[] = __( 'You must supply a SolidWP membership username and password in order to change the licensed URL.', 'it-l10n-ithemes-security-pro' );
		} else if ( empty( $data['username'] ) ) {
	...
	private function show_notices( $is_staging = false ) {
		if ( $is_staging) {
			echo "<div class=\"updated\"><p><strong>" . __( 'As a staging site, licenses for the products and services listed below do not count toward your license usage.', 'it-l10n-ithemes-security-pro' ) . "</strong></p></div>\n";
		}
