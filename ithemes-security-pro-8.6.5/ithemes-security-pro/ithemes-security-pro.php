...
if ( version_compare( phpversion(), '7.4.0', '<' ) ) {
	function itsec_minimum_php_version_notice() {
		echo '<div class="notice notice-error"><p>' . esc_html__( 'Solid Security requires PHP 7.4 or higher.', 'it-l10n-ithemes-security-pro' ) . '</p></div>';
	}
...
$activate_callback = static function() use ( &$activate_callback ) {
	...
	if ( in_array( $free_plugin, $active_plugins, true ) ) {
		...
		wp_die(
			esc_html__(
				'Solid Security Pro cannot be activated because Solid Security Basic is already active. Please deactivate Solid Security Basic first.',
